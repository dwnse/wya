...
function MemberRequest() {
    ...
    const [form, setForm] = useState({ nombre_usuario: '', nombre_mostrar: '', minecraft_username: '', fecha_ingreso: '', razon: '' })
    ...
    async function handleSubmit(event) {
        ...
        if (!user?.id || form.razon.trim().length < 20 || !form.nombre_usuario.trim() || !form.nombre_mostrar.trim() || !form.fecha_ingreso) return
        try {
            setLoading(true)
            setError('')
            setSolicitud(await solicitarMembresia(user.id, { ...form, email: user.email }))
            setForm({ nombre_usuario: '', nombre_mostrar: '', minecraft_username: '', fecha_ingreso: '', razon: '' })
        } catch (requestError) {
    ...
    if (!isLoggedIn) {
        ...
        content = <div className="member-request-panel"><div className="panel-title"><span>Solicitar membresía</span><Icon name="file" size={17} /></div>{solicitud?.estado === 'rechazada' && <p className="request-status request-rejected">Tu solicitud fue rechazada. Puedes enviar una nueva.</p>}<form onSubmit={handleSubmit}><label htmlFor="request-name">Nombre de usuario / Nombre visible</label><input id="request-name" value={form.nombre_usuario} onChange={event => setForm({ ...form, nombre_usuario: event.target.value, nombre_mostrar: event.target.value })} maxLength={50} required /><label htmlFor="request-minecraft">Minecraft username</label><input id="request-minecraft" value={form.minecraft_username} onChange={event => setForm({ ...form, minecraft_username: event.target.value })} maxLength={50} placeholder="Steve" /><label htmlFor="request-date">Fecha de ingreso</label><input id="request-date" type="date" value={form.fecha_ingreso} onChange={event => setForm({ ...form, fecha_ingreso: event.target.value })} required /><label htmlFor="member-reason">¿Por qué quieres ser miembro?</label><textarea id="member-reason" value={form.razon} onChange={event => setForm({ ...form, razon: event.target.value })} minLength={20} maxLength={1000} placeholder="Escribe la razón de tu solicitud" required /><small>{form.razon.length}/1000 caracteres. Mínimo 20.</small><button className="discord-cta" type="submit" disabled={loading || form.razon.trim().length < 20 || !form.nombre_usuario.trim() || !form.fecha_ingreso}>{loading ? 'Enviando...' : 'Enviar solicitud'}</button>{error && <p className="request-error">{error}</p>}</form></div>
    }
